...
@interface YCWebViewController () <NSURLConnectionDataDelegate>
...
- (void) setupView {
    ...
    UIColor* titleColor = [UIColor redColor];//[UIColor colorWithRed:0 green:146/255.0 blue:1.0 alpha:1.0]
    //地址栏
    _textField = [[UITextField alloc]initWithFrame:CGRectMake(10, 50, 300, 25)];
    _textField.borderStyle = UITextBorderStyleRoundedRect;
    _textField.textColor = titleColor;
    _textField.text = @"http://dlsw.baidu.com/sw-search-sp/soft/b1/38200/WeChat_1.5.0.33_setup.1446175356.exe";
    ...
    //下载按钮
    _button = [[UIButton alloc]initWithFrame:CGRectMake(10, 200, 300, 25)];
...
#pragma mark - 发送数据请求
- (void) sendRequest {
    NSString *urlStr = _textField.text;
    //注意对于url中的中文是无法解析的，需要进行url编码（指定编码类型为utf-8）
    //另外注意url解码使用stringByRemoveingPercentEncoding方法
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    NSString *filename = [_textField.text lastPathComponent];
